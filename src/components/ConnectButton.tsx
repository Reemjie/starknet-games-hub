...
export function ConnectButton() {
  ...
  useEffect(() => {
    ...
    const getUserName = async () => {
      try {
        if (cartridgeConnector.username) {
          const u = await cartridgeConnector.username();
          setUsername(u ?? null);
        }
      } catch (_) {}
    };
    ...
    <div className="w-wrapper" ref={ref}>
      ...
      {open && (
        <div className="w-dropdown">
          ...
          <a className="w-dropdown__item" href="https://cartridge.gg/profile" target="_blank" rel="noreferrer">
            🌐 Cartridge Profile ↗
          </a>
          <hr className="w-dropdown__sep" />
          <button className="w-dropdown__item w-dropdown__item--red"
